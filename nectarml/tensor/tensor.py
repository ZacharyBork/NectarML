...
class tensor:
    ...
    def reshape(self: tensor, shape: tuple[builtins.int, ...]) -> Self:
        self_requires_grad = self.requires_grad
    ...
    def view(self: tensor, shape: tuple[builtins.int, ...]) -> Self:
        total = self.numel()
        if -1 in shape:
            idx = shape.index(-1)
            known = 1
    ...
    def permute(self: tensor, dims: tuple[builtins.int, ...] | None) -> Self:
        self_requires_grad = self.requires_grad
    ...
    def expand(
        self: tensor, 
        shape: tuple[builtins.int, ...] | typing.Size
    ) -> Self:
    ...
    def broadcast_to(self: tensor, shape: tuple[builtins.int, ...]) -> Self:
    ...
    def select(self: tensor, dim: builtins.int, index: builtins.int) -> Self:
