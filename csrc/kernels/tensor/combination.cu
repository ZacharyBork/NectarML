...
template<typename T>
void launch_stack(ConcatInputs inputs, TensorIndex out_idx, T* out, int dim, size_t n_elements) {
    int block = BLOCK_SIZE_1D;
    int grid = (n_elements + block - 1) / block;
    stack_kernel<T><<<grid, block>>>(inputs, out_idx, out, dim, n_elements);
}

template void launch_stack<float>(ConcatInputs, TensorIndex, float*, int, size_t);
template void launch_stack<half>(ConcatInputs, TensorIndex, half*, int, size_t);
template void launch_stack<uint8_t>(ConcatInputs, TensorIndex, uint8_t*, int, size_t);
template void launch_stack<int32_t>(ConcatInputs, TensorIndex, int32_t*, int, size_t);
