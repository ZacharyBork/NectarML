...
void launch_reduce_min(
    ...
    switch (threads) {
        case 512: reduce_min_kernel<T, 512><<< blocks, threads, smemSize >>>(in_data, out_data, in_idx, out_idx, reduce_dim); break;
        case 256: reduce_min_kernel<T, 256><<< blocks, threads, smemSize >>>(in_data, out_data, in_idx, out_idx, reduce_dim); break;
        case 128: reduce_min_kernel<T, 128><<< blocks, threads, smemSize >>>(in_data, out_data, in_idx, out_idx, reduce_dim); break;
        case 64:  reduce_min_kernel<T,  64><<< blocks, threads, smemSize >>>(in_data, out_data, in_idx, out_idx, reduce_dim); break;
        case 32:  reduce_min_kernel<T,  32><<< blocks, threads, smemSize >>>(in_data, out_data, in_idx, out_idx, reduce_dim); break;
        case 16:  reduce_min_kernel<T,  16><<< blocks, threads, smemSize >>>(in_data, out_data, in_idx, out_idx, reduce_dim); break;
        case 8:   reduce_min_kernel<T,   8><<< blocks, threads, smemSize >>>(in_data, out_data, in_idx, out_idx, reduce_dim); break;
        case 4:   reduce_min_kernel<T,   4><<< blocks, threads, smemSize >>>(in_data, out_data, in_idx, out_idx, reduce_dim); break;
        case 2:   reduce_min_kernel<T,   2><<< blocks, threads, smemSize >>>(in_data, out_data, in_idx, out_idx, reduce_dim); break;
        case 1:   reduce_min_kernel<T,   1><<< blocks, threads, smemSize >>>(in_data, out_data, in_idx, out_idx, reduce_dim); break;
    }
}

template void launch_reduce_min<float>(float*, float*, TensorIndex, TensorIndex, int);
template void launch_reduce_min<half>(half*, half*, TensorIndex, TensorIndex, int);
template void launch_reduce_min<uint8_t>(uint8_t*, uint8_t*, TensorIndex, TensorIndex, int);
template void launch_reduce_min<int32_t>(int32_t*, int32_t*, TensorIndex, TensorIndex, int);
